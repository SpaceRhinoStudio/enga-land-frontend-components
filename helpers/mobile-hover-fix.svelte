...
<script lang="ts" context="module">
      ...
      try {
        if (sheet.cssRules) {
          for (let ruleIndex = sheet.cssRules.length - 1; ruleIndex >= 0; ruleIndex--) {
            ...
            if (!rule) {
              console.error("Couldn't access cssRule at index ", ruleIndex, ' of ' + sheet)
              continue
            }
            //@ts-expect-error real object differs from typings
            rule.selectorText = rule.selectorText?.replace(searchValue, replaceValue)
          }
        }
      } catch {
  ...
  function updateStyleSheets(mediaQueryList: MediaQueryList | MediaQueryListEvent) {
